...
})({"1hb9a":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "24f019b31cf3d105";
"use strict";
...
},{}],"27bAL":[function(require,module,exports,__globalThis) {
...
parcelHelpers.export(exports, "replaceCards", ()=>replaceCards);
parcelHelpers.export(exports, "fetchStandingsData", ()=>fetchStandingsData);
parcelHelpers.export(exports, "replaceStandingsCards", ()=>replaceStandingsCards);
async function fetchStandingsData() {
    ...
    try {
        ...
        if (!standingsData.drivers_championship || !Array.isArray(driversData)) throw new Error("\u274C Datos corruptos o vac\xedos.");
        ...
        console.error("\u274C Error al obtener standings con im\xe1genes:", error.message);
        return [];
    }
}
...
function replaceStandingsCards(standingsData) {
    ...
    standingsData.forEach((driver, index)=>{
        ...
        cards[index].querySelector("h3").textContent = `\u{1F3C6} ${driver.driver.name} - ${driver.team.teamName}`;
        cards[index].querySelector("p").textContent = `Posici\xf3n: ${driver.position} | Puntos: ${driver.points}`;
        cards[index].querySelector("a").href = driver.driver.url || "#";
    });
    console.log("\u2705 Standings con im\xe1genes actualizados correctamente.");
}
...
async function fetchFilteredData(filter = "All Drivers") {
    ...
    try {
        ...
        if (!apiUrl) throw new Error(`\u{274C} Filtro no v\xe1lido: ${filter}`);
        const response = await fetch(apiUrl);
        if (!response.ok) throw new Error(`Error ${response.status}: No se pudo obtener la data.`);
        let data = await response.json();
        if (!data || Object.keys(data).length === 0) throw new Error(`\u{274C} Datos vac\xedos o corruptos para ${filter}`);
        replaceCards(data.slice(0, 8), filter);
    } catch (error) {
        console.error(`\u{274C} Error al obtener datos de ${filter}:`, error.message);
    }
}
...
function replaceCards(data, filter) {
    ...
    cards.forEach((card, index)=>{
        ...
        if (filter === "All Drivers") {
            ...
            card.querySelector("h3").textContent = `\u{1F3CE}\u{FE0F} ${item.nombre} ${item.apellido} - ${item.team}`;
            card.querySelector("p").textContent = `\u{1F4C6} Nacimiento: ${item.fechaNacimiento} | \u{1F1EC}\u{1F1E7} Nacionalidad: ${item.nacionalidad}`;
            card.querySelector("a").href = `https://www.formula1.com/en/drivers/${item.driverId}.html` || "#";
        } else if (filter === "F1 Teams") {
            card.querySelector("h3").textContent = `\u{1F3CE}\u{FE0F} ${item.nombre} - ${item.nacionalidad}`;
            card.querySelector("p").textContent = `Fundado: ${item.primeraAparicion} | \u{1F4CD} Sede: ${item.nacionalidad}`;
            ...
            card.querySelector("h3").textContent = `\u{1F4CD} ${item.nombre} - ${item.pais}`;
            card.querySelector("p").textContent = `Ubicaci\xf3n: ${item.ciudad} | \u{1F3C1} Longitud: ${item.longitud} km`;
            card.querySelector("img").src = item.urlImagen;
            card.querySelector("a").href = item.url || "#";
        }
    });
    console.log(`\u{2705} Datos actualizados con el filtro: ${filter}`);
}
...
document.addEventListener("DOMContentLoaded", async ()=>{
    ...
    filtersContainer.addEventListener("news-selected", async (event)=>{
        ...
        console.log(`\u{1F50E} Aplicando filtro: ${selectedFilter}`);
        if (selectedFilter === "F1 Standings") {
            const standingsData = await fetchStandingsData();
            if (standingsData.length > 0) replaceStandingsCards(standingsData);
        } else await fetchFilteredData(selectedFilter);
    });
    // 🔥 Cargar datos iniciales con `"All Drivers"`
    fetchFilteredData("All Drivers");
});

},{"@parcel/transformer-js/src/esmodule-helpers.js":"kjvkr"}],"kjvkr":[function(require,module,exports,__globalThis) {
...
},{}]},["1hb9a","27bAL"], "27bAL", "parcelRequirecff4", {})

//# sourceMappingURL=resutsRacing.1cf3d105.js.map
